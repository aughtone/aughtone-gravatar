...
- **No skill files.** This repository does not use machine-readable agent skills. Do not add `*.ai-skill.md`, `META-INF/ai-skills/`, `META-INF/agents/skills/` or a `.agents/` directory, do not ship them in the published artifacts, and do not scan dependencies for them.
